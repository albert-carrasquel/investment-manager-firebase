...
// Estas variables se proporcionan automáticamente en el entorno de ejecución
const rawAppId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';

// Uso una limpieza robusta para asegurar que el ID de la aplicación sea un único segmento 
// de ruta válido para Firestore, reemplazando cualquier caracter de ruta inválido (/ , . , :) por un guion.
const appId = rawAppId.replace(/[/\.:]/g, '-'); 
const firebaseConfig = typeof __firebase_config !== 'undefined' 
    ? JSON.parse(__firebase_config) 
    : {};
// Inicialización del token de autenticación
const initialAuthToken = typeof __initial_auth_token !== 'undefined' 
    ? __initial_auth_token 
    : null;

// Rutas de Firestore (Usando la estructura pública de Canvas para compartir datos)
// Ruta: artifacts (C) / appId (D) / public (C) / data (D) / transactions (C)
...
const App = () => {
    ...
    // 1. Inicialización de Firebase y Autenticación (Flujo controlado y esperado)
    useEffect(() => {
        ...
        const runAuthAndInit = async () => {
            try {
                ...
                // Paso 1: Intentar autenticación (await asegura que la promesa termine antes de continuar)
                if (initialAuthToken) {
                    const userCredential = await signInWithCustomToken(firebaseAuth, initialAuthToken);
                    user = userCredential.user;
                    console.log(`Autenticación exitosa (Custom Token). User ID: ${user.uid}`);
                } else {
                    const userCredential = await signInAnonymously(firebaseAuth);
                    user = userCredential.user;
                    console.log(`Autenticación exitosa (Anónima). User ID: ${user.uid}`);
                }
                
                // Paso 2: Establecer estados críticos DESPUÉS de la autenticación exitosa
                if (user) {
                    setUserId(user.uid);
                }
                ...
                // Usamos e.message para dar más detalle si falla la autenticación
                setError(`Authentication failed: ${e.message}. Check console for details.`);
            } finally {
                // Paso 3: Indicar que la inicialización y autenticación inicial han finalizado.
                // Esto desbloquea el useEffect de Firestore
                setIsAuthReady(true);
                setIsLoading(false);
            }
        };
    ...
    // 2. Carga de Transacciones en Tiempo Real (onSnapshot)
    useEffect(() => {
        // Ejecución protegida: Solo si la autenticación ha terminado Y db existe Y userId es válido
        if (!isAuthReady || !db || !userId) return;

        const transactionsPath = getTransactionsCollectionPath(appId);
        console.log(`Firestore Path being used for onSnapshot (App ID: ${appId}, User ID: ${userId}): ${transactionsPath}`); 
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            snapshot.forEach((doc) => {
                ...
                fetchedTransactions.push({
                    id: doc.id,
                    ...data,
                    // Asegurar que timestamp se muestre correctamente
            ...
            if (err.code === 'permission-denied') {
                 // Este error indica una falta de permisos en las reglas de seguridad de Firestore, no un error de código.
                 // Cambiamos el mensaje de error para que sea más genérico y la solución se muestre en el componente.
                 setError("Error: Acceso bloqueado. El error 'permission-denied' indica que necesitas actualizar las Reglas de Seguridad de Firestore.");
            } else {
                 setError("Error fetching transactions: Problema de red o configuración.");
            }
        });

        return () => unsubscribe();
    }, [db, userId, isAuthReady]); // Depende de que db, userId y la señal de Auth Ready estén establecidos
    ...
    // Manejo de la Nueva Transacción
    ...
    const handleAddTransaction = async (e) => {
        e.preventDefault();
        // Validación sin usar alert()
        if (!db || !userId || !newTransaction.amount || !newTransaction.name) {
            setFormError('Por favor, completa el nombre y el monto antes de agregar la transacción.');
            return;
        }

        try {
            const transactionsPath = getTransactionsCollectionPath(appId);
            // FIX: Agregamos un log más detallado para el path
            console.log(`Firestore Path being used for addDoc (App ID: ${appId}, User ID: ${userId}): ${transactionsPath}`); 
    ...
    // Función para eliminar una transacción (tras confirmación)
    ...
    if (error) {
        // ERROR DE PÉRMISO: Se muestra el snippet de la regla de seguridad como solución
        return (
            <div className="p-8 bg-red-100 border border-red-400 text-red-700 rounded-lg max-w-lg mx-auto mt-10 shadow-xl">
                ...
                <p className="mt-4 text-sm font-bold text-red-800">
                    ACCIÓN REQUERIDA: El error `permission-denied` indica
                    que las Reglas de Seguridad de tu base de datos Firestore están bloqueando el acceso.
                    Para una aplicación colaborativa pública como esta, debes permitir la lectura y escritura.
                </p>
                
                <h3 className="mt-4 text-lg font-semibold text-red-700">Regla de Seguridad Sugerida (para copiar):</h3>
                
                <div className="mt-3 p-4 bg-red-200 rounded-lg text-xs break-all overflow-x-auto font-mono">
  ...
  match /databases/{database}/documents {
    ...
    // }
                ...
                <p className="mt-4 text-xs text-red-800 italic">
                    Copia y pega este *snippet* en el editor de "Reglas" de tu consola de Firebase 
                    para solucionar el problema de permisos.
                </p>
                
                <div className="mt-4 p-2 bg-red-300 rounded text-xs break-all">
                    **Ruta de Firestore utilizada:** {getTransactionsCollectionPath(appId)}
                    <br />
            ...
            <header className="mb-8 p-4 bg-white shadow-lg rounded-xl">
                ...
                {userId && (
                    <div className="mt-3 text-xs text-gray-600 p-2 bg-indigo-50 rounded-lg break-all">
                        **Tu ID de Usuario (para colaboración):** {userId}
                    </div>
                )}
            </header>
                    ...
                    <div className="md:col-span-3 bg-white p-6 shadow-xl rounded-xl">
                        ...
                        <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                            {transactions.length === 0 ? (
                                <p className="text-gray-500 italic">Aún no hay transacciones. ¡Agrega la primera!</p>
                            ) : (
                                transactions.map((t) => (
                                    <TransactionItem 
                                        key={t.id} 
        ...
        <div className={`bg-white p-6 rounded-xl shadow-2xl transform hover:scale-[1.02] transition duration-300 ease-in-out`}>
            <div className="flex justify-between items-start">
                <div>
                    ...
                    <h3 className={`text-3xl font-extrabold mt-1 text-gray-900 ${color === 'red' && amount < 0 ? 'text-red-600' : 'text-gray-900'}`}>
...
const TransactionItem = ({ transaction, formatCurrency, onDelete }) => {
    ...
    const typeClass = isInvestment ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700';
    const Icon = isInvestment ? ArrowUpRight : ArrowDownLeft;

    const formattedDate = transaction.timestamp instanceof Date 
        ...
        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200 hover:shadow-md transition duration-150 ease-in-out">
            <div className="flex items-center space-x-3 min-w-0">
                ...
                <div className="min-w-0">
                    <p className="font-semibold text-gray-800 truncate">{transaction.name}</p>
                    <p className="text-xs text-gray-500">{formattedDate}</p>
                </div>
            </div>
            
            <div className="flex items-center space-x-3">
                <p className={`font-bold text-lg ${isInvestment ? 'text-green-600' : 'text-red-600'}`}>
